  ...
  Fl_Group(0, 0, W, H, L)
{
  ...
  { cache_bttn = new Fl_Button(508, 43, 20, 20);
    cache_bttn->box(FL_NO_BOX);
    cache_bttn->down_box(FL_THIN_UP_FRAME);
    cache_bttn->hide();
  } // Fl_Button* cache_bttn
  { moreinfo_bttn = new Fl_Button(535, 43, 20, 20);
    moreinfo_bttn->tooltip(_("Show more information about this video."));
    moreinfo_bttn->box(FL_NO_BOX);
    moreinfo_bttn->down_box(FL_THIN_UP_FRAME);
  } // Fl_Button* moreinfo_bttn
...
MoreVideoInfo::MoreVideoInfo(int X, int Y, int W, int H, const char *L) :
  Fl_Double_Window(X, Y, W, H, L)
{
  _MoreVideoInfo();
}

MoreVideoInfo::MoreVideoInfo(int W, int H, const char *L) :
  Fl_Double_Window(0, 0, W, H, L)
{
  clear_flag(16);
  _MoreVideoInfo();
...
MoreVideoInfo::MoreVideoInfo() :
  Fl_Double_Window(0, 0, 547, 466, 0)
{
  clear_flag(16);
  _MoreVideoInfo();
}

void MoreVideoInfo::_MoreVideoInfo() {
  this->box(FL_FLAT_BOX);
  this->color(FL_BACKGROUND_COLOR);
  this->selection_color(FL_BACKGROUND_COLOR);
  this->labeltype(FL_NO_LABEL);
  ...
  this->align(Fl_Align(FL_ALIGN_TOP));
  this->when(FL_WHEN_RELEASE);
  { vm_thumbnail = new Fl_Box(108, 8, 330, 190);
    vm_thumbnail->box(FL_UP_FRAME);
  } // Fl_Box* vm_thumbnail
  { vm_scroll = new Fl_Scroll(0, 200, 547, 266);
    vm_scroll->box(FL_NO_BOX);
    vm_scroll->color(FL_BACKGROUND_COLOR);
    vm_scroll->selection_color(FL_BACKGROUND_COLOR);
    vm_scroll->labeltype(FL_NORMAL_LABEL);
    vm_scroll->labelfont(0);
    vm_scroll->labelsize(14);
    vm_scroll->labelcolor(FL_FOREGROUND_COLOR);
    vm_scroll->align(Fl_Align(65));
    vm_scroll->when(FL_WHEN_RELEASE);
    vm_scroll->type(Fl_Scroll::VERTICAL);
    vm_scroll->scrollbar.align(FL_ALIGN_RIGHT | FL_ALIGN_TOP);
    vm_scroll->end();
  } // Fl_Scroll* vm_scroll
  { vm_retrieving_info = new Fl_Box(22, 218, 503, 30, _("Retrieving video metadata..."));
    vm_retrieving_info->labelfont(3);
    vm_retrieving_info->hide();
  } // Fl_Box* vm_retrieving_info
  set_modal();
  this->label(_("Detailed Video Metadata"));
  end();
}

/**
 Clear all video metadata added to this window.
*/
void MoreVideoInfo::delete_all_metadata() {
  metadata_list.clear();
  vm_scroll->clear();
}

/**
 Add a new metadata struct. Key is the "metadata title". Does not check if previously was added...
*/
void MoreVideoInfo::add_metadata(std::string k, std::string v) {
  if (k != "") {
      video_metadata vm;
      vm.key = k;
      vm.value = v;
      metadata_list.push_back(vm);
  }
}

/**
 Print metadata on screen.
*/
void MoreVideoInfo::print_metadata() {
  const int fixed_box_width = this->vm_scroll->w() - 20;
      int dyn_box_height = 0;
      int box_x = this->vm_scroll->x() + 10;
      int box_y = this->vm_scroll->y() + 10;

      for (video_metadata mtd: metadata_list) {
          //std::cout << mtd.key << " + " << mtd.value << "\n";
          //std::cout << "X: " << box_x << " Y: " << box_y << "\n";
          Fl_Box* box_key = new Fl_Box(box_x, box_y, fixed_box_width, 20);
          box_key->labelfont(FL_HELVETICA_BOLD);
          box_key->copy_label(mtd.key.c_str());
          box_key->align(FL_ALIGN_LEFT | FL_ALIGN_INSIDE | FL_ALIGN_WRAP);
          fl_font(this->vm_scroll->labelfont(), this->vm_scroll->labelsize());
          int box_width = fixed_box_width;
          fl_measure(mtd.value.c_str(), box_width, dyn_box_height, 1);
          //std::cout << "X: " << box_x << " Y: " << box_y + box_key->h() << " Box WIDTH: " << fixed_box_width << " HEIGHT: " << dyn_box_height <<"\n";
          Fl_Box* box_value = new Fl_Box(box_x, box_y + box_key->h(), fixed_box_width, dyn_box_height);
          box_value->copy_label(mtd.value.c_str());
          box_value->align(FL_ALIGN_LEFT | FL_ALIGN_INSIDE | FL_ALIGN_WRAP);
          box_y = box_y + 20 + dyn_box_height + 10;
          this->vm_scroll->add(box_key);
          this->vm_scroll->add(box_value);
      }
}
